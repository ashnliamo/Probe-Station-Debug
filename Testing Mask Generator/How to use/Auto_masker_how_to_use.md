...
outputs/
  ...
  calibration/
    calibration_resistors.gds   a coupon on the SAME pad ring as the test chips, so
                                the probe card lands on it directly; its resistors
                                hang off the output pads and each value is written
                                beside the pad it belongs to
    calibration_resistors.csv   which two pads to measure between, and the value
                                that resistor should read
...
automatically from a CSV:
  ...
  1. Land the probe card on the calibration chip. It has the same pad ring as a test
     chip, so it drops straight on. The resistors hang off the OUTPUT pads, which are
     not shorted to each other externally, so each one reads on its own: one output
     pad is COMMON, and every other listed pad reaches it through exactly one
     resistor. The CSV gives the pair to measure in its measure_from and measure_to
     columns, and each value is also written next to its pad on the layout. Measure
     COMMON to each listed pad in turn and note the reading.

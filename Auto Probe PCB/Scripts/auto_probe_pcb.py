...
# name shared by the PcbLib footprint, the SchLib component, and the cleanup
# script that deletes it -- these must match for the symbol/footprint link and
# for the cleanup to find its target.
...
def _sch_desig(netclass):
    safe = "".join(c if c.isalnum() else "_" for c in netclass.upper())
    return safe.strip("_") or "NET"


def write_schematic_script(path, L):
    # Emit a standalone .SchDoc: one vertical net-class block per class, each
    # pin one probe land (designator = land id, name = signal). A separate
    # readable sheet, distinct from the reusable SchLib component.
    classes = group_by_netclass(L["probes"])
    ox, oy = SCH_ORIGIN

    o = []
    w = o.append
    w("""// Auto-generated by auto_probe_pcb.py -- Altium schematic DelphiScript.
...
""")
    ...
    for ci, (netclass, members) in enumerate(classes.items()):
        ...
        top = oy
        proc = f"BuildBlock{ci}"
        build_calls.append(proc)
        w(f"\nProcedure {proc};\nVar C;\nBegin")
        w(f"    AddTitle({x}, {top + SCH_TITLE_GAP}, {_pas_str(netclass)});")
        w(f"    C := NewBlock({x}, {top}, {SCH_BLOCK_WIDTH}, {height}, "
          f"{_pas_str(_sch_desig(netclass))});")
        for k, pr in enumerate(members):
            py = top - (k + 1) * SCH_PIN_PITCH
            sig = pr["die"]["signal"] or pr["id"]
            w(f"    AddPin(C, {x}, {py}, {_pas_str(pr['id'])}, {_pas_str(sig)});")
        w("    Commit(C);\nEnd;")
        x += SCH_BLOCK_WIDTH + SCH_PIN_LENGTH + SCH_BLOCK_GAP

    w("\nProcedure GenerateSchematic;\nBegin")
    w("""    If SchServer = Nil Then
    Begin
    ...
    for proc in build_calls:
        w(f"    {proc};")
    w("""    SchServer.ProcessControl.PostProcess(Doc, '');
...
Begin
    ...
    FP.Name := """ + _pas_str(PROBECARD_NAME) + """;
    BuildPads;
    Lib.RegisterComponent(FP);
    Lib.CurrentComponent := FP;
    Lib.Board.ViewManager_FullUpdate;
    ShowMessage('Footprint """ + PROBECARD_NAME + """ built in the library. Save the .PcbLib.');
End;""")
...
""")
    ...
    #
    # Idempotency: Lib.AddSchComponent always ADDs -- it never replaces a
    # same-named component, so re-running piles copies under one 'ProbeCard'
    # (the "N overlapping rectangles per part" bug). RemoveExisting clears any
    # prior ProbeCard first. NOTE the iterator API asymmetry confirmed from
    # working Altium scripts: create with SchLibIterator_Create, but destroy
    # with SchIterator_Destroy (SchLibIterator_Destroy does not exist). The
    # loop variable MUST be typed ISch_Component or .LibReference won't compile.
    w("""
...
Begin
    Try
        ...
        Begin
            ...
            If Cmp.LibReference = """ + _pas_str(PROBECARD_NAME) + """ Then
...
Begin
    ...
    RemoveExisting;
    Comp := SchServer.SchObjectFactory(eSchComponent, eCreate_GlobalCopy);
    Comp.LibReference := {_pas_str(PROBECARD_NAME)};
    ...
    Impl.ModelName := """ + _pas_str(PROBECARD_NAME) + """;
...
def write_sch_cleanup_script(path):
    # Standalone purge: delete EVERY component named PROBECARD_NAME from the
    # active .SchLib. GenerateSymbol already calls its own RemoveExisting, so
    # this is the manual escape hatch -- use it to clear a library that has
    # accumulated copies from earlier runs, or if RemoveExisting silently
    # failed on this Altium version. Reports how many it deleted, which also
    # tells you how many copies had piled up.
    #
    # Same verified iterator pattern as RemoveExisting: create with
    # SchLibIterator_Create, destroy with SchIterator_Destroy (note the
    # asymmetry -- SchLibIterator_Destroy does not exist), loop variable typed
    # ISch_Component so .LibReference resolves, and step to the next object
    # BEFORE deleting the current one. UNVERIFIED against a live Altium.
    name = _pas_str(PROBECARD_NAME)
    o = []
    w = o.append
    w("""// Auto-generated by auto_probe_pcb.py -- Altium Sch-library DelphiScript.
// Removes every '""" + PROBECARD_NAME + """' component from the active schematic library.
...
Begin
    ...
    If Count = 0 Then
        ShowMessage('No """ + PROBECARD_NAME + """ components found -- library is already clean.')
    Else
        ShowMessage('Removed ' + IntToStr(Count) + ' """ + PROBECARD_NAME +
        """ component(s). Save the .SchLib to keep the change.');
End;""")
...
def main():
    ...
    sch_path = OUTPUT_DIR / f"{csv_path.stem}_schematic.pas"
    write_schematic_script(sch_path, L)
    ...
    clean_path = OUTPUT_DIR / f"{csv_path.stem}_probecard_cleanup.pas"
    write_sch_cleanup_script(clean_path)
    print(f"Wrote Sch-lib cleanup:   {clean_path}")
